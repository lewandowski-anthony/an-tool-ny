...
This directory contains a highly optimized and secure `Dockerfile` designed to containerize **Spring Boot** applications (Java 25).

It leverages the **Multi-Stage Build** pattern combined with Spring Boot's **Layer Tools** to deliver lightning-fast builds, maximized Docker cache utilization, and hardened production-grade security.
...
* **Observability (Datadog)**: Natively embeds the Datadog Java APM Agent (`dd-java-agent.jar`) for seamless performance monitoring.
* **Proxy / AWS Network Bypass**: Features an environment variable slot to inject enterprise proxy configurations, custom certificates, or specific AWS endpoints.

---

## 📦 Application Prerequisites

Before triggering the Docker build, your Spring Boot application must be configured to support layer splitting.
...
### 🔹 Simple Local Run
To spin up your application mapping port `8080`:
```bash
